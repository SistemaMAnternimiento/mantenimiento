...
namespace dbMantenimientoBiz
{
	...
	public abstract class detalle_reparoOrmTemplate : Business
	{
		...
		/// <summary>
		/// Gets and Sets the parent dbMantenimientoBiz.materialusado.
		/// </summary>
		 
		public virtual dbMantenimientoBiz.materialusado materialusado
		{
			get
			{
				if (row.GetParentRow(base.DataSet.Relations["idmaterialusadodetalle_reparo"]) != null)
					return new dbMantenimientoBiz.materialusado( DataContext, row.GetParentRow("idmaterialusadodetalle_reparo"));
				else
					return null;
			}
			set
